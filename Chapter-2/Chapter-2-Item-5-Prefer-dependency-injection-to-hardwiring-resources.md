...
许多类依赖于一个或多个底层资源。例如，拼写检查类依赖于字典类。常见做法是，将这种类实现为静态工具类（[Item-4](../Chapter-2/Chapter-2-Item-4-Enforce-noninstantiability-with-a-private-constructor.md)）：
...
你可以尝试让 SpellChecker 支持多个字典：首先取消 dictionary 字段的 final 修饰，并在现有的拼写检查类中添加更改 dictionary 的方法。但是在并发环境中这种做法是笨拙的、容易出错的和不可行的。**静态工具类和单例不适用于由底层资源参数化的类。**
